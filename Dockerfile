...
ADD ./continuous_deployment/startup_scripts/01_db_migrate.sh /etc/my_init.d/01_db_migrate.sh
CMD ["/sbin/my_init"]
...
ADD ./continuous_deployment/environment-variables.conf /etc/nginx/main.d/environment-variables.conf

# Passenger Configuration & App
RUN rm /etc/nginx/sites-enabled/default
ADD ./continuous_deployment/fedora_ingest_rails.conf /etc/nginx/sites-enabled/fedora_ingest_rails.conf
